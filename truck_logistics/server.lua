...
Citizen.CreateThread(function()
	...
	while true do 
		...
		for k,v in pairs(data) do
			...
			local source = vRP.getUserSource({tonumber(v.user_id)})
...
AddEventHandler("truck_logistics:finishJob",function(data,distance,reward,truck_data,truck_engine,truck_body,trailer_body)
	...
	if user_id then
		...
		MySQL.Sync.execute(sql, {['@reward'] = (reward+bonus)*trailer_body, ['@distance'] = tonumber(string.format("%.2f", distance)), ['@exp_amount'] = (exp_amount+bonus_exp)*trailer_body, ['@user_id'] = user_id});



		giveTruckerMoney(user_id,(reward+bonus)*trailer_body)

		TriggerClientEvent("Notify",source,"sucesso",Lang[Config.lang]['reward']:format(tostring((reward+bonus)*trailer_body),tostring(trailer_body*100),tostring(exp_amount+bonus_exp)*trailer_body))
